...
impl Linear {
    pub fn new(input_shape: usize, output_shape: usize, activation_function: ActivationFunction, activation_derivative: ActivationFunction) -> Self {
        Linear {
    ...
    pub fn forward(&self, input: &Array2<f32>) -> Array2<f32> {
        (self.activation_function)(&(self.weights.dot(input) + &self.bias))
    }

    pub fn backward(&self, 
        input: &Array2<f32>, 
        output_gradient: &Array2<f32>, 
        mut weight_gradient: Array2<f32>, 
        mut bias_gradient: Array2<f32>
    ) -> Array2<f32> {
        // Compute the derivative of the activation function then combine with the output gradient to get the delta
        let delta = output_gradient * (self.activation_derivative)(input);
...
impl Layer {
    pub fn linear(input_size: usize, output_size: usize, activation_function: ActivationFunction, activation_derivative: ActivationFunction) -> Self {
        Layer::Linear(Linear::new(input_size, output_size, activation_function, activation_derivative))
    }
}
